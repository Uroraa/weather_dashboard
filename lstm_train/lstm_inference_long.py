...
class LSTMModelLong(nn.Module):
    def __init__(self):
        ...
        # Output chỉ temperature + humidity
...
def inverse_transform_output(scaler, arr: np.ndarray) -> np.ndarray:
    """
    arr shape: (N, 2) — scaled temperature & humidity.
    Padding về 6 cột để dùng scaler, rồi lấy lại 2 cột đầu.
    """
    pad = np.zeros((arr.shape[0], 4))
    return scaler.inverse_transform(np.hstack([arr, pad]))[:, :2]
...
def evaluate_real_error(device_id: int = DEVICE_ID):
    ...
    for i in range(len(scaled) - WINDOW_SIZE - HORIZON + 1):
        ...
        y_true = scaled[i + WINDOW_SIZE : i + WINDOW_SIZE + HORIZON, :2]
    ...
    for key, steps in HORIZON_OPTIONS.items():
        ...
        print(f"  [{key:>3s}] MAE nhiệt độ: {mae_temp:.3f} °C | MAE độ ẩm: {mae_humi:.3f} %")
